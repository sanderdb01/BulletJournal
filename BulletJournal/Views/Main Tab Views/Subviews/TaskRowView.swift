...
struct TaskRowView: View {
      ...
      HStack(spacing: 16) {
         ...
         VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 8) {
               ...
               // Anchor indicator with lineage
               if task.isAnchor == true {
                  HStack(spacing: 4) {
                     Image(systemName: "ferry.fill")
                        .font(.caption)
                     
                     // Show day count if carried over
                     if let dayCount = task.anchorDayCount, dayCount > 1 {
                        Text("Day \(dayCount)")
                           .font(.caption)
                           .fontWeight(.medium)
                     }
   ...
   // MARK: - Share Row View
   @ViewBuilder
   private var shareRowView: some View {
         ...
         HStack {
            ...
            if let participants = task.shareParticipantNames, !participants.isEmpty {
               if participants.count > 1 {
                  Text("Shared with \(participants.count) People")
                     .font(.subheadline)
                     .foregroundColor(.secondary)
               } else {
   ...
   private func checkDailyReminderStatus() {
      ...
      if !hasIncompleteTasks {
         ...
         Task {
            ...
            print("🎉 All tasks complete - cancelled today's reminder")
         }
      }
   }
   
   // MARK: - Haptic feedback for tap and hold
   private func startAcceleratingHaptics() {
      let startTime = Date()
      ...
      hapticTimer = Timer.scheduledTimer(withTimeInterval: 0.05, repeats: true) { timer in
         ...
         // Fire haptics more frequently as progress increases
         // Early: ~4 per second, Late: ~20 per second
         let shouldFire = Double.random(in: 0...1) < progress * 4
